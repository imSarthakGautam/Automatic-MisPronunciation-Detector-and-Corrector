...
function SubmitAudioText() {
  ...
  const [textToSpeak, setTextToSpeak] = useState(""); // Text input for what to speak
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("text", textToSpeak); // Add text for comparison
      ...
      const response = await fetch("/api/transcribe-compare", {
        // Updated endpoint for comparison
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        if (result.result && Array.isArray(result.result)) {
          // Format comparison for red/green highlights (correct/incorrect)
          const formattedTranscription = result.result
            ...
            .join(" ");
          setTranscription(formattedTranscription); // Update transcription with colored HTML
        } else {
...
export default SubmitAudioText;
